...
export default function Library() {
  ...
  const [filterOpen, setFilterOpen] = useState(false) // 手机端悬浮球

  // 新增表单
  const [newCard, setNewCard] = useState({ title: '', content: '', category: '', author: '', tags: '' })
  ...
  async function fetchCards() {
    ...
    if (mainSelected.length > 0) {
      query = query.in('category', mainSelected)
    }
  ...
  async function fetchAllTags() {
    ...
    data.forEach((r: any) => r.tags?.forEach((t: string) => {
      if (!MAIN_CATEGORIES.includes(t)) tagSet.add(t)
    }))
...
function toggleTag(tag: string) {
  setSelectedTags(prev => {
    const cur = prev[tag]
    if (!cur) return { ...prev, [tag]: 'include' }        // 第一次：选中
    if (cur === 'include') return { ...prev, [tag]: 'exclude' }  // 第二次：反选
    const next = { ...prev }; delete next[tag]; return next      // 第三次：取消
  })
  setPage(1)
}
  async function addCard() {
    ...
    setSaving(true)
    const tags = newCard.tags.split(/[,，\s]+/).filter(Boolean)
  ...
  const tagButtonStyle = (tag: string) => {
  ...
  return {
    ...
    background: state === 'include' ? '#1a1a1a' : '#f5f5f3',
    color: state === 'include' ? '#fff' : '#999',
    borderColor: state === 'include' ? '#1a1a1a' : '#e8e8e6',
    ...
    <div style={{ fontSize: '13px' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '20px' }}>
        {MAIN_CATEGORIES.map(c => (
          <button key={c} onClick={() => toggleTag(c)} style={tagButtonStyle(c)}>{c}</button>
        ))}
      </div>
      {allTags.length > 0 && (
        <>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
            {(tagsExpanded ? allTags : allTags.slice(0, 6)).map(t => (
              <button key={t} onClick={() => toggleTag(t)} style={tagButtonStyle(t)}>{t}</button>
            ))}
          </div>
          {allTags.length > 6 && (
            <button onClick={() => setTagsExpanded(v => !v)}
              style={{ marginTop: '8px', fontSize: '11px', color: '#aaa', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
      ...
      {Object.keys(selectedTags).length > 0 && (
        <button onClick={() => { setSelectedTags({}); setPage(1) }}
          style={{ marginTop: '16px', fontSize: '11px', color: '#aaa', background: 'none', border: 'none', cursor: 'pointer', padding: 0, display: 'block' }}>
          清空选中
        </button>
      )}
    </div>
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#fafaf8', fontFamily: 'Inter, sans-serif' }}>
      ...
      <aside className="sidebar-desktop" style={{
        ...
        background: '#fff', borderRight: '1px solid #f0f0ee',
        display: 'flex', flexDirection: 'column', padding: '32px 0', zIndex: 40,
      }}>
        <div style={{ padding: '0 20px 28px', fontFamily: 'Noto Serif SC,serif', fontSize: '20px', fontWeight: 300, letterSpacing: '0.2em', color: '#1a1a1a' }}>
          Yuria
        </div>
      ...
      {/* 主内容区 */}
      <div className="main-content" style={{ marginLeft: '220px', flex: 1, padding: '32px 40px', paddingBottom: '80px' }}>
        {/* 顶部栏 */}
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '8px' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: '15px', fontWeight: 500, color: '#1a1a1a', marginBottom: '4px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {card.title}
                    </div>
                  ...
                  <button onClick={e => { e.stopPropagation(); deleteCard(card.id) }}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px 4px', color: '#ddd', flexShrink: 0, marginLeft: '8px' }}
                ...
                <div style={{
                  ...
                  overflow: 'hidden',
                  maxHeight: expandedId === card.id ? 'none' : '72px',
                  maskImage: expandedId === card.id ? 'none' : 'linear-gradient(to bottom, black 60%, transparent)',
                  WebkitMaskImage: expandedId === card.id ? 'none' : 'linear-gradient(to bottom, black 60%, transparent)',
                ...
                {card.tags?.length > 0 && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginBottom: '12px' }}>
                    {card.tags.map(t => (
                      <span key={t} style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '4px', background: '#f5f5f3', color: '#888' }}>{t}</span>
                    ))}
                  </div>
                )}
                ...
                <div style={{ display: 'flex', gap: '8px' }} onClick={e => e.stopPropagation()}>
                  <button onClick={() => copyContent(card.content, card.id)}
                    style={{
                      ...
                      border: '1px solid #ebebeb', background: copied === card.id ? '#f0fdf4' : '#fff',
                      color: copied === card.id ? '#22c55e' : '#666',
                    ...
                    {copied === card.id ? (
                      ...
                      <><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg> 复制</>
                    )}
                  </button>
                  <button onClick={() => router.push(`/notes/library/${card.id}`)}
                    style={{
        ...
        {totalPages > 1 && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginTop: '32px', flexWrap: 'wrap' }}>
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
              style={{ padding: '7px 14px', borderRadius: '8px', border: '1px solid #ebebeb', background: '#fff', color: page === 1 ? '#ccc' : '#333', cursor: page === 1 ? 'not-allowed' : 'pointer', fontSize: '13px' }}>
              ← 上一页
            </button>
            {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
              ...
              return (
                <button key={p} onClick={() => setPage(p)}
                  style={{ width: '36px', height: '36px', borderRadius: '8px', border: '1px solid', borderColor: p === page ? '#1a1a1a' : '#ebebeb', background: p === page ? '#1a1a1a' : '#fff', color: p === page ? '#fff' : '#333', cursor: 'pointer', fontSize: '13px' }}>
                  {p}
                </button>
              )
            })}
            {totalPages > 5 && <span style={{ color: '#ccc' }}>...</span>}
            <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}
              style={{ padding: '7px 14px', borderRadius: '8px', border: '1px solid #ebebeb', background: '#fff', color: page === totalPages ? '#ccc' : '#333', cursor: page === totalPages ? 'not-allowed' : 'pointer', fontSize: '13px' }}>
              下一页 →
            </button>
      ...
      <nav className="mobile-nav" style={{
        ...
        borderTop: '0.5px solid #ebebeb', padding: '10px 0',
        paddingBottom: 'max(10px,env(safe-area-inset-bottom))',
        justifyContent: 'space-around', zIndex: 50,
      }}>
        {[
          { label: '词库', path: '/notes/library', icon: <><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></> },
          { label: '记录', path: '/notes/records', icon: <><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><path d="M14 2v6h6"/></> },
        ].map(item => (
          <button key={item.path} onClick={() => router.push(item.path)}
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '3px', background: 'none', border: 'none', cursor: 'pointer', flex: 1, color: item.path === '/notes/library' ? '#1a1a1a' : '#bbb' }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round">{item.icon}</svg>
      ...
      <button className="filter-float" onClick={() => setFilterOpen(v => !v)}
        style={{
          ...
          background: Object.keys(selectedTags).length > 0 ? '#1a1a1a' : '#fff',
          border: '1px solid #e8e8e6', boxShadow: '0 4px 16px rgba(0,0,0,0.12)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          cursor: 'pointer', zIndex: 40,
          color: Object.keys(selectedTags).length > 0 ? '#fff' : '#666',
        }}>
      ...
      {showAdd && (
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{ fontFamily: 'Noto Serif SC,serif', fontWeight: 300, fontSize: '18px', marginBottom: '20px', color: '#1a1a1a' }}>新增小剧场</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {[
                { key: 'title', placeholder: '标题 *', label: '标题' },
                { key: 'author', placeholder: 'Yuria', label: '作者' },
              ].map(({ key, placeholder, label }) => (
                <div key={key}>
                  <label style={{ fontSize: '12px', color: '#aaa', marginBottom: '6px', display: 'block' }}>{label}</label>
                  <input value={(newCard as any)[key]} onChange={e => setNewCard(p => ({ ...p, [key]: e.target.value }))}
              ...
              <div>
                <label style={{ fontSize: '12px', color: '#aaa', marginBottom: '6px', display: 'block' }}>标签（逗号分隔）</label>
                <input value={newCard.tags} onChange={e => setNewCard(p => ({ ...p, tags: e.target.value }))}
                  placeholder="校园, 甜文, 现代..."
                  style={{ width: '100%', border: '1px solid #ebebeb', borderRadius: '10px', padding: '10px 14px', fontSize: '13px', outline: 'none' }} />
              </div>
              <div>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
                <button onClick={() => setShowAdd(false)}
                  style={{ flex: 1, padding: '11px', borderRadius: '10px', border: '1px solid #ebebeb', background: '#fff', color: '#666', fontSize: '13px', cursor: 'pointer' }}>
                  取消
                </button>
                <button onClick={addCard} disabled={saving || !newCard.title}
                  style={{ flex: 2, padding: '11px', borderRadius: '10px', border: 'none', background: saving || !newCard.title ? '#f0f0ee' : '#1a1a1a', color: saving || !newCard.title ? '#aaa' : '#fff', fontSize: '13px', cursor: saving || !newCard.title ? 'not-allowed' : 'pointer' }}>
                  {saving ? '保存中...' : '保存'}
                </button>
              </div>
            </div>
