...
function TagInput({ tags, onChange }: { tags: string[], onChange: (tags: string[]) => void }) {
  ...
  return (
    <div style={{
      ...
      border: '1px solid #ebebeb', borderRadius: '10px', padding: '8px 12px',
      minHeight: '42px', cursor: 'text',
    }} onClick={e => (e.currentTarget.querySelector('input') as HTMLInputElement)?.focus()}>
      {tags.map(t => (
        <span key={t} style={{
          display: 'inline-flex', alignItems: 'center', gap: '4px',
          background: '#f5f5f3', borderRadius: '6px',
          padding: '2px 8px', fontSize: '12px', color: '#555',
        }}>
          {t}
...
export default function Library() {
  ...
  const tagBtnStyle = (tag: string) => {
    ...
    return {
      ...
      background: s === 'include' ? '#1a1a1a' : s === 'exclude' ? '#fee2e2' : '#f5f5f3',
      color: s === 'include' ? '#fff' : s === 'exclude' ? '#ef4444' : '#666',
      borderColor: s === 'include' ? '#1a1a1a' : s === 'exclude' ? '#fca5a5' : '#e8e8e6',
  ...
  const FilterPanel = () => (
    <div style={{ fontSize: '13px' }}>
      ...
      {allTags.length > 0 && <>
        ...
        {allTags.length > 6 && <button onClick={() => setTagsExpanded(v => !v)} style={{ marginTop: '8px', fontSize: '11px', color: '#aaa', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
  ...
  const ModalForm = ({ isEdit }: { isEdit: boolean }) => {
    const form = isEdit ? editForm : newCard
    const setForm = isEdit
      ? (fn: any) => setEditForm(fn)
      : (fn: any) => setNewCard(fn)
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        {!isEdit && <div>
          ...
          <input value={(form as any).author} onChange={e => setForm((p: any) => ({ ...p, author: e.target.value }))}
            placeholder="Yuria"
            style={{ width: '100%', border: '1px solid #ebebeb', borderRadius: '10px', padding: '10px 14px', fontSize: '13px', outline: 'none' }} />
        </div>}
        <div>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {MAIN_CATEGORIES.map(c => (
              <button key={c} onClick={() => setForm((p: any) => ({ ...p, category: p.category === c ? '' : c }))}
                style={{ padding: '6px 14px', borderRadius: '8px', border: '1px solid', fontSize: '12px', cursor: 'pointer', background: form.category === c ? '#1a1a1a' : '#f5f5f3', color: form.category === c ? '#fff' : '#666', borderColor: form.category === c ? '#1a1a1a' : '#ebebeb' }}>
                {c}
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <TagInput tags={form.tags} onChange={tags => setForm((p: any) => ({ ...p, tags }))} />
        </div>
        <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '8px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '4px', marginLeft: '8px', flexShrink: 0 }}>
                      <button onClick={e => openEdit(card, e)}
                        style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '3px', color: '#ccc' }}
      ...
      {showAdd && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }} onClick={() => setShowAdd(false)}>
          <div style={{ background: '#fff', borderRadius: '20px', padding: '28px', width: 'min(480px,100%)', maxHeight: '85vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }} onClick={e => e.stopPropagation()}>
            <h2 style={{ fontFamily: 'Noto Serif SC,serif', fontWeight: 300, fontSize: '18px', marginBottom: '20px', color: '#1a1a1a', margin: '0 0 20px' }}>新增小剧场</h2>
