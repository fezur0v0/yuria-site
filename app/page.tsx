  ...
  return (
    <>
      <style>{`
        ...
        html,body{font-family:'Inter',sans-serif;background:#f5f5f3;margin:0;padding:0}
        .sr{opacity:0;transform:translateY(24px) scale(.99);filter:blur(3px);
        ...
        @keyframes br{0%,100%{opacity:.15;transform:scale(.6)}50%{opacity:.7;transform:scale(1.2)}}
        .bdot{display:inline-block;width:4px;height:4px;border-radius:50%;background:#1a1a1a;margin:0 6px;vertical-align:middle;animation:br 2.8s ease-in-out infinite}
        .bdot2{animation-delay:.9s}
        @keyframes vspin{to{transform:rotate(360deg)}}
        .vinyl-playing{animation:vspin 3s linear infinite}
        ...
        .wv-play:nth-child(1){animation:w1 .7s ease-in-out infinite}
        .wv-play:nth-child(2){animation:w2 .5s ease-in-out infinite .1s}
        .wv-play:nth-child(3){animation:w3 .8s ease-in-out infinite .05s}
        .wv-play:nth-child(4){animation:w4 .6s ease-in-out infinite .15s}
        .wv-play:nth-child(5){animation:w5 .75s ease-in-out infinite .08s}
      `}</style>

      {track && <audio ref={audioRef} src={track.src} onEnded={() => setTrackIdx(i => (i+1)%tracks.length)} />}

      <main className="max-w-3xl mx-auto bg-white min-h-screen pb-32">
        ...
        <div className="relative h-80 overflow-hidden">
          ...
          <div className="absolute inset-0" style={{background:'linear-gradient(to bottom,rgba(255,255,255,0) 0%,rgba(255,255,255,0) 20%,rgba(255,255,255,.18) 45%,rgba(255,255,255,.65) 70%,rgba(255,255,255,.95) 92%,#fff 100%)'}} />
          <div className="absolute bottom-0 left-0 right-0 px-12 pb-7 flex items-end justify-between z-10">
            <div>
              <div style={{fontFamily:'Noto Serif SC,serif',fontSize:'48px',fontWeight:300,letterSpacing:'.14em',lineHeight:1,color:'#1a1a1a'}}>Yuria</div>
            ...
            {user
              ? <div className="flex gap-2 items-center">
                  <Link href="/admin" className="text-xs text-gray-400 border border-gray-200 rounded-full px-3 py-1.5 hover:border-gray-400 transition-colors">管理</Link>
                  <button onClick={signOut} className="text-xs text-gray-400 border border-gray-200 rounded-full px-3 py-1.5 hover:border-gray-400 transition-colors">退出</button>
                </div>
        ...
        {tracks.length > 0 && (
          <div className="sr px-12" data-d="0">
            <div className="flex items-center gap-3 py-3 cursor-pointer border-b border-gray-50" onClick={() => setListOpen(o => !o)}>
              <div className={`w-9 h-9 rounded-full flex-shrink-0 relative ${playing ? 'vinyl-playing' : ''}`}
              ...
              <div className="flex gap-1.5 items-center flex-shrink-0">
                {[0,1,2,3,4].map(i => (
                  <div key={i} className={`w-0.5 rounded-sm bg-gray-300 ${playing ? 'wv-play' : ''} ${playing ? 'bg-gray-800' : ''}`}
              ...
              <div className="flex gap-1 flex-shrink-0">
                <button className="w-7 h-7 rounded-full border border-gray-200 flex items-center justify-center hover:bg-gray-50 transition-colors"
                  onClick={e => { e.stopPropagation(); setTrackIdx(i => (i-1+tracks.length)%tracks.length) }}>
                  <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M8 2L4 5l4 3V2zM2 2h1v6H2V2z" fill="#555"/></svg>
                </button>
                <button className="w-7 h-7 rounded-full border border-gray-200 flex items-center justify-center bg-gray-100 hover:bg-gray-200 transition-colors"
                  onClick={e => { e.stopPropagation(); togglePlay() }}>
                  {playing
                    ? <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><rect x="2" y="2" width="2.5" height="6" rx="1" fill="#1a1a1a"/><rect x="5.5" y="2" width="2.5" height="6" rx="1" fill="#1a1a1a"/></svg>
                    : <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M3 2l5 3-5 3V2z" fill="#1a1a1a"/></svg>
                  }
                </button>
                <button className="w-7 h-7 rounded-full border border-gray-200 flex items-center justify-center hover:bg-gray-50 transition-colors"
                  onClick={e => { e.stopPropagation(); setTrackIdx(i => (i+1)%tracks.length) }}>
                  <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M2 2l4 3-4 3V2zM7 2h1v6H7V2z" fill="#555"/></svg>
                </button>
              </div>
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none" className="flex-shrink-0 text-gray-300" style={{transform: listOpen ? 'rotate(180deg)' : 'none', transition:'transform .3s'}}>
            ...
            <div style={{maxHeight: listOpen ? '200px' : '0', overflow:'hidden', transition:'max-height .4s cubic-bezier(.22,1,.36,1)'}}>
              {tracks.map((t, i) => (
                <div key={t.id} className="flex items-center gap-3 py-2.5 border-t border-gray-50 cursor-pointer hover:bg-gray-50 px-1 -mx-1 rounded"
            ...
            {[
              ...
              <Link key={i} href="/gallery"
                ...
                <div className="relative h-24">
                  {[{rot:'-3deg',op:.45},{rot:'1.5deg',op:.7},{rot:'0',op:1}].map((l,j) => (
                    <div key={j} className="absolute rounded-lg overflow-hidden border-2 border-white"
                      style={{left:'5%',width:'90%',top:`${j*6}px`,transform:`rotate(${l.rot})`,zIndex:j+1}}>
                      <div style={{height:'62px',background:item.bg,opacity:l.op}}/>
                    </div>
                  ))}
                </div>
                <div className="text-[10px] text-gray-400 mt-1.5 px-0.5">{item.label}</div>
              </Link>
            ))}
          </div>
        </div>
        ...
        <div className="sr px-12 pt-9 pb-4" data-d="140">
          ...
          <button onClick={openPw}
            ...
            <div className="w-9 h-9 rounded-full bg-gray-200 flex items-center justify-center flex-shrink-0 group-hover:bg-gray-300 transition-colors">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><rect x="3" y="6" width="8" height="6" rx="1.5" stroke="#666" strokeWidth="1.2"/><path d="M5 6V4.5a2 2 0 014 0V6" stroke="#666" strokeWidth="1.2" strokeLinecap="round"/></svg>
            </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 flex justify-center"
        style={{background:'rgba(255,255,255,.93)',backdropFilter:'blur(18px)',borderTop:'0.5px solid #f0f0f0',paddingBottom:'max(14px,env(safe-area-inset-bottom))'}}>
        <div className="w-full max-w-3xl flex justify-around py-2.5">
          {[
            {href:'/',label:'主页',icon:<svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M2 8l7-6 7 6v8H11v-4H7v4H2V8z" stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round"/></svg>},
            {href:'/portfolio',label:'作品集',icon:<svg width="18" height="18" viewBox="0 0 18 18" fill="none"><rect x="2" y="2" width="6" height="6" rx="1.5" stroke="currentColor" strokeWidth="1.3"/><rect x="10" y="2" width="6" height="6" rx="1.5" stroke="currentColor" strokeWidth="1.3"/><rect x="2" y="10" width="6" height="6" rx="1.5" stroke="currentColor" strokeWidth="1.3"/><rect x="10" y="10" width="6" height="6" rx="1.5" stroke="currentColor" strokeWidth="1.3"/></svg>},
            {href:'/gallery',label:'图集',icon:<svg width="18" height="18" viewBox="0 0 18 18" fill="none"><rect x="2" y="2" width="14" height="14" rx="2" stroke="currentColor" strokeWidth="1.3"/><circle cx="6.5" cy="6.5" r="1.5" stroke="currentColor" strokeWidth="1.3"/><path d="M2 12l4-3 3 3 2-2 5 4" stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round"/></svg>},
          ].map(item => (
            <Link key={item.href} href={item.href} className="flex flex-col items-center gap-1 text-gray-300 hover:text-gray-700 transition-colors px-4">
              {item.icon}
          ...
          <button onClick={openPw} className="flex flex-col items-center gap-1 text-gray-300 hover:text-gray-700 transition-colors px-4">
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><rect x="3" y="8" width="12" height="8" rx="1.5" stroke="currentColor" strokeWidth="1.3"/><path d="M6 8V6a3 3 0 016 0v2" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg>
      ...
      {/* 密码弹窗 */}
      {pwOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center" style={{background:'rgba(0,0,0,.28)',backdropFilter:'blur(10px)'}} onClick={closePw}>
          <div className="bg-white rounded-2xl p-8 w-68 text-center" style={{boxShadow:'0 20px 60px rgba(0,0,0,.15)',width:'272px'}} onClick={e => e.stopPropagation()}>
            ...
            <div className="flex justify-center gap-3 mb-4">
              {[0,1,2,3].map(i => (
                <div key={i} className="w-2.5 h-2.5 rounded-full border transition-all duration-200"
                  style={{background:pw.length>i?'#1a1a1a':'transparent',borderColor:pw.length>i?'#1a1a1a':'#ddd'}}/>
              ))}
            </div>
            ...
            <input ref={inputRef} type="password" maxLength={4} value={pw}
              ...
              placeholder="· · · ·"
            />
            <button onClick={checkPw} className="w-full rounded-xl py-2.5 text-sm tracking-widest text-white mb-3 hover:opacity-90 active:scale-95 transition-all"
              style={{background:'#1a1a1a'}}>进入</button>
            <button onClick={closePw} className="text-xs text-gray-400 hover:text-gray-600 transition-colors">取消</button>
          </div>
        </div>
      )}
    </>
