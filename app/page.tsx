      ...
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        .sb-link{
          ...
          color:#585858;font-size:13px;letter-spacing:.08em;
        ...
        .sb-bar{
          ...
          width:2px;height:14px;background:#b0b0b0;border-radius:2px;
        ...
        .sidebar{
          ...
          padding:52px 0 36px;
          z-index:50;
          box-shadow:4px 0 48px rgba(0,0,0,.14);
        }
    ...
    audioRef.current?.play().catch(() => {})
      ...
      <div className="layout">
        ...
        <aside className="sidebar">
          {/* Logo */}
          <div style={{
            fontFamily:'Noto Serif SC,serif',fontSize:'24px',fontWeight:300,
            letterSpacing:'.22em',color:'#f0f0ee',
            padding:'0 32px',marginBottom:'56px',
          }}>Yuria</div>
        ...
        <main className="main-area">
          ...
          <div className="hero-section" style={{position:'relative',height:'420px',overflow:'hidden'}}>
            ...
            <div style={{position:'absolute',inset:0,background:
              'linear-gradient(to bottom,rgba(250,250,248,0) 0%,rgba(250,250,248,0) 15%,rgba(250,250,248,.08) 38%,rgba(250,250,248,.38) 60%,rgba(250,250,248,.82) 80%,rgba(250,250,248,1) 100%)'
            }}/>
            <div className="content-wrap" style={{position:'absolute',bottom:0,left:0,right:0,padding:'0 56px 30px',zIndex:2}}>
              <div className="hero-name-txt" style={{
                fontFamily:'Noto Serif SC,serif',fontSize:'60px',fontWeight:300,
                letterSpacing:'.16em',color:'#1a1a1a',lineHeight:1,
              }}>Yuria</div>
          ...
          <div className="player-wrap sr" style={{padding:'0 56px',borderBottom:'0.5px solid #efefed'}} data-d="0">
            <div style={{display:'flex',alignItems:'center',gap:'16px',padding:'18px 0',cursor:'pointer'}}
              ...
              <div style={{display:'flex',alignItems:'flex-end',gap:'3px',height:'22px',flexShrink:0}}>
                {[5,11,7,15,9].map((h,i) => (
                  <div key={i} className={playing ? 'wv-on' : ''}
                    style={{width:'2.5px',borderRadius:'2px',
              ...
              <div style={{display:'flex',alignItems:'center',gap:'8px',flexShrink:0}}>
                <button
                  onClick={e=>{e.stopPropagation();setTrackIdx(i=>(i-1+tracks.length)%tracks.length);setPlaying(false)}}
                  style={{width:'33px',height:'33px',borderRadius:'50%',border:'1px solid #d8d8d8',
                    background:'#fff',display:'flex',alignItems:'center',justifyContent:'center',
                    cursor:'pointer',transition:'border-color .2s,background .2s'}}
                  onMouseEnter={e=>{e.currentTarget.style.borderColor='#888';e.currentTarget.style.background='#f5f5f3'}}
                  onMouseLeave={e=>{e.currentTarget.style.borderColor='#d8d8d8';e.currentTarget.style.background='#fff'}}>
                ...
                <button
                  ...
                  {playing
                    ? <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                        <rect x="3" y="2.5" width="2" height="7" rx="1" fill="#fff"/>
                        <rect x="7" y="2.5" width="2" height="7" rx="1" fill="#fff"/>
                      </svg>
                    : <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                        <path d="M3.5 2.5l6 3.5-6 3.5V2.5z" fill="#fff"/>
                      </svg>
                  }
                </button>
                <button
...
setPlaying(true)}}
              ...
              <svg width="13" height="13" viewBox="0 0 13 13" fill="none"
                style={{flexShrink:0,transition:'transform .3s',transform:listOpen?'rotate(180deg)':'none'}}>
                <path d="M2 4.5l4.5 4.5 4.5-4.5" stroke="#c0c0c0" strokeWidth="1.5" strokeLinecap="round"/>
              </svg>
            </div>

            {/* 曲目列表 */}
            <div style={{maxHeight:listOpen?'220px':'0',overflow:'hidden',
              transition:'max-height .4s cubic-bezier(.22,1,.36,1)'}}>
              {tracks.map((t,i) => (
                <div key={t.id}
                  onClick={()=>{setTrackIdx(i);if(!playing)togglePlay()}}
                  style={{display:'flex',alignItems:'center',gap:'14px',padding:'11px 4px',
                    borderTop:'0.5px solid #f2f2f0',cursor:'pointer',borderRadius:'6px',
...
marginBottom:'42px'}}>
            ...
            <div className="g-grid" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'24px'}}>
              {galleryItems.map((item,i) => (
                <Link key={i} href="/gallery" className="sr"
                  data-d={`${110+i*28}`}
                  ...
                  <div style={{position:'relative',paddingBottom:'66.67%',margin:'4px'}}>
                    {[
                      {rot:'-3.2deg',op:.42,top:'0%'},
                      {rot:'1.5deg', op:.68, top:'6%'},
                      ...
                      <div key={j} style={{
                        ...
                        boxShadow:`0 ${2+j*3}px ${10+j*6}px rgba(0,0,0,${.06+j*.02})`,
                        aspectRatio:'3/2',
                      }}>
                        <div className="g-img-inner"
                          style={{width:'100%',paddingBottom:'66.67%',background:item.bg,opacity:l.op}}/>
                      </div>
                    ))}
                  </div>
      ...
      {/* ════ PASSWORD MODAL ════ */}
          ...
          <div className="pw-fadein"
            style={{background:'rgba(252,252,250,.97)',borderRadius:'24px',
              padding:'48px 44px',width:'320px',textAlign:'center',
              boxShadow:'0 36px 90px rgba(0,0,0,.18)',
            }}
            onClick={e=>e.stopPropagation()}>

            <div style={{fontFamily:'Noto Serif SC,serif',fontSize:'17px',fontWeight:300,
...
marginBottom:'42px'}}>
            ...
            <input ref={inputRef} type="password" value={pw}
              ...
              style={{
                width:'100%',border:'0.5px solid #e0e0e0',borderRadius:'14px',
                padding:'14px',textAlign:'center',letterSpacing:'.2em',
                fontSize:'16px',marginBottom:'18px',background:'transparent',
            ...
            <button onClick={checkPw} disabled={checking}
              style={{
                ...
                letterSpacing:'.18em',cursor:checking?'default':'pointer',
                marginBottom:'14px',fontFamily:'Inter,sans-serif',
                opacity:checking?.65:1,transition:'opacity .2s,background .2s',
              }}
              onMouseEnter={e=>{if(!checking)(e.currentTarget as HTMLElement).style.background='#383838'}}
              onMouseLeave={e=>{if(!checking)(e.currentTarget as HTMLElement).style.background='#1a1a1a'}}>
              {checking ? '验证中…' : '进 入'}
            </button>
